  ...
  return (
    <div>
      ...
      >
        ...
        <span className={`flex-none w-1.5 h-1.5 rounded-full shrink-0 ${ok ? "bg-emerald-500" : "bg-destructive"}`} />
        <span className={`shrink-0 font-mono font-medium ${ok ? "text-foreground" : "text-destructive"}`}>
          {node.function}
        </span>
        <span className="flex-1 border-b border-dotted border-border" />
        <span className="shrink-0 font-mono text-muted-foreground/50 text-[10px]">{node.id}</span>
        <div className="relative h-1.5 w-20 shrink-0 rounded-full bg-muted overflow-hidden">
          <div
            className={`absolute left-0 top-0 h-full rounded-full transition-all ${ok ? "bg-primary/60" : "bg-destructive/60"}`}
