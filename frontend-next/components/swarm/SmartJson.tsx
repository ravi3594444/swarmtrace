...
function tryParse(raw: string): unknown | undefined {
  ...
  try { return JSON.parse(trimmed); } catch {
    try {
      return JSON.parse(trimmed.replace(/'/g, '"').replace(/\bTrue\b/g, "true").replace(/\bFalse\b/g, "false").replace(/\bNone\b/g, "null"));
...
export function SmartJson({ raw, maxHeight }: { raw: string; maxHeight?: string }) {
  ...
  if (parsed === undefined) {
    return (
      <pre className="overflow-auto rounded-lg border border-border bg-muted/40 p-3 text-xs text-foreground" style={{ maxHeight }}>
        {raw || "(empty)"}
      </pre>
    );
  }
  return (
    <div className="overflow-auto rounded-lg border border-border bg-muted/40 p-3 text-xs" style={{ maxHeight }}>
...
function JsonNode({ value, depth, keyName }: { value: unknown; depth: number; keyName: string | null }) {
  ...
  if (entries.length === 0) return <Line keyEl={renderKey}><span className="text-muted-foreground">{open_c}{close_c}</span></Line>;
      ...
      <div className="flex items-start gap-1">
        ...
        <div className="flex-1">
          ...
          <span className="text-muted-foreground">
            {open_c}
            {!open && <span className="text-muted-foreground/70"> …{entries.length} </span>}
            {!open && close_c}
          </span>
          {open && (
            <div className="ml-3 border-l border-border pl-2">
              {entries.map(([k, v]) => <JsonNode key={k} value={v} depth={depth + 1} keyName={isArr ? null : k} />)}
            </div>
          )}
...
function Line({ keyEl, children }: { keyEl: React.ReactNode; children: React.ReactNode }) {
  return (
    <div className="pl-4">
      {keyEl}{keyEl && <span className="text-muted-foreground">: </span>}
      {children}
    </div>
  );
}