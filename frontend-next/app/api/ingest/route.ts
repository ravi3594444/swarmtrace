...
export async function POST(req: Request) {
  ...
  try {
    // 1. Resolve API key (stored only as a hash) and map it to user_id
    const keyHash = crypto.createHash('sha256').update(apiKey).digest('hex')
    const keys = await supaRequest(`api_keys?key_hash=eq.${keyHash}&revoked=eq.false&limit=1`)
    ...
    try {
      await supaRequest(`api_keys?key_hash=eq.${keyHash}`, {
        method: 'PATCH',
