...
class RuleEngine:
    ...
    def evaluate(self, traces: List[TraceRow]) -> List[Alert]:
        ...
        for row in traces:
            agent_id = row[12] or row[0] or "unknown"
    ...
    def _rule_budget_breach(
        ...
        for agent_id, rows in by_agent.items():
            ...
            agent_name = None
            for row in rows:
                ts = self._parse_ts(row[7])
                if ts is None or ts < cutoff:
                    continue
                total += float(row[10] or 0.0)
                trace_ids.append(row[0])
                if latest_ts is None or ts > latest_ts:
                    latest_ts = ts
                agent_name = agent_name or row[13]
    ...
    def _rule_error_spike(self, by_agent: Dict[str, List[TraceRow]]) -> List[Alert]:
        ...
        for agent_id, rows in by_agent.items():
            ...
            if len(recent) < cfg.min_traces:
                continue
            errors = sum(1 for r in recent if r[6])   # row[6] = error
            rate = errors / len(recent)
            if rate >= cfg.error_rate_threshold:
                agent_name = next((r[13] for r in recent if r[13]), agent_id)
                fired.append(Alert(
                    id=uuid.uuid4().hex,
                    rule="error_spike",
                    ...
                    trace_ids=[r[0] for r in recent if r[6]][:50],
                ))
    ...
    def _rule_latency_regression(self, by_agent: Dict[str, List[TraceRow]]) -> List[Alert]:
        ...
        for agent_id, rows in by_agent.items():
            ...
            if len(recent) < cfg.min_traces:
                continue
            latencies = sorted(
                float(r[5] or 0.0) for r in recent
            )
            ...
            if p95 >= cfg.latency_p95_sec:
                ...
                fired.append(Alert(
                    ...
                    trace_ids=[r[0] for r in recent[:50]],
                ))
